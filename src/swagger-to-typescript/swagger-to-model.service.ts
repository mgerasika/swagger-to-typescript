...
export const getMethods = (source: any, models: ISwaggerModel[]): ISwaggerMethod[] => {
            ...
            return Object.keys(path).map((httpMethod: string) => {
                ...
                return {
                    ...
                    body: {
                        ...
                        model: models.find(
                            (x) => x.schema === content?.requestBody?.content['application/json']?.schema['$ref'],
                        ),
                    },
                    successResponse: Object.keys(content.responses)
                        ...
                        .map((statusStr) => getSwaggerMethodReturn(content, statusStr, models))
                        .filter((f) => f)[0],
                    ...
                    errorResponses: Object.keys(content.responses)
                        ...
                        .map((statusStr) => getSwaggerMethodReturn(content, statusStr, models) as ISwaggerMethodReturn)
                        .filter((f) => f),
                };
            });
        })
...
export const getModel = (root: any, schema: any, key: string): ISwaggerModel => {
    ...
    return {
        ...
        originalName: key,
        type: schema.type,
        schema: root.components.schemas[key] ? `#/components/schemas/${key}` : undefined,
        enum: schema.enum,
        arrayItemModel,
        properties: properties
            ? Object.keys(properties).map((propertyKey: string) => {
                  ...
                  const propertyType = property.type === 'integer' ? 'number' : property.type || subModel?.name;
                  return {
                      name: propertyKey,
                      type: propertyType,
...
const getPropertyModel = (root: any, propertySchema: any, propertyKey: string): ISwaggerModel | undefined => {
...
const getMethodParameters = (methodName: string, content: any): ISwaggerMethodParameter[] | undefined => {
    return content.parameters
        ? content.parameters.map((p: any) => {
              const enumName = p.schema.enum ? 'E' + capitalize(methodName) + '_' + capitalize(p.name) : undefined;
              return {
                  name: p.name,
                  in: p.in,
                  required: p.required,
                  type: enumName || (p.schema.type === 'integer' ? 'number' : p.schema.type),
                  subModel: p.schema.enum ? getPropertyModel(p, { ...p.schema, type: enumName }, p.name) : undefined,
